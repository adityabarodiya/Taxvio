...
function GSTBadge({ rate }: { rate: number }) {
  return (
    <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold border ${RATE_COLORS[rate] ?? "bg-gray-100 text-gray-600 border-gray-200"}`}>
      {rate === 0 ? "Exempt / NIL" : `${rate}% GST`}
    </span>
  );
}
...
// ─── Result Card ─────────────────────────────────────────────────────────────
function HSNCard({ entry, isExpanded, onToggle }: {
  entry: HSNEntry;
  isExpanded: boolean;
  onToggle: () => void;
}) {
  return (
    <motion.div
      layout
      className={`border rounded-2xl overflow-hidden transition-all duration-200 ${isExpanded ? "border-[#00416a]/30 shadow-md" : "border-gray-200 hover:border-[#00416a]/20 hover:shadow-sm"}`}
    >
      <button
        onClick={onToggle}
        className="w-full text-left p-4 flex items-start gap-3 hover:bg-gray-50/60 transition"
      >
        {/* HSN Badge */}
        <div className="shrink-0 mt-0.5">
          <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${RATE_BG_STRONG[entry.gstRate] ?? "bg-gray-500"}`}>
            <span className="text-white text-[9px] font-extrabold leading-tight text-center">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-2 mb-1">
            <span className="font-mono text-sm font-extrabold text-[#00416a] tracking-wide">
              {entry.hsn}
            </span>
            <span className={`text-[10px] px-2 py-0.5 rounded-full font-semibold ${entry.type === "goods" ? "bg-blue-50 text-blue-600" : "bg-violet-50 text-violet-600"}`}>
              {entry.type === "goods" ? "📦 Goods" : "🛠️ Service"}
            </span>
            <span className="text-[10px] bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full font-medium hidden sm:inline">
              {entry.category}
            </span>
          </div>
          <p className="text-sm text-gray-700 font-medium leading-snug line-clamp-2 text-left">
            {entry.description}
          </p>
        </div>
        ...
        <div className="shrink-0 flex items-center gap-1.5">
          ...
          <div className={`w-7 h-7 rounded-full flex items-center justify-center transition-all duration-300 ${isExpanded ? "bg-[#00416a] rotate-180" : "bg-gray-100"}`}>
            <ChevronDown size={14} className={isExpanded ? "text-white" : "text-gray-500"} />
          </div>
        </div>
      </button>
                ...
                <div className="space-y-2.5">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1">
                      <span className="font-mono font-extrabold text-[#00416a] text-sm">{entry.hsn}</span>
                      <CopyButton text={entry.hsn} />
                    </div>
                  </div>

                  <div>
                    <p className="text-[10px] text-gray-400 font-semibold uppercase tracking-wide mb-0.5">Description</p>
                    <p className="text-xs text-gray-700 leading-relaxed">{entry.description}</p>
                  </div>

                  <div>
                    <p className="text-[10px] text-gray-400 font-semibold uppercase tracking-wide mb-0.5">Chapter / Classification</p>
                    <p className="text-xs text-gray-600 font-medium">{entry.chapter}</p>
                  </div>

                  <div>
                    <p className="text-[10px] text-gray-400 font-semibold uppercase tracking-wide mb-0.5">Category</p>
                    <p className="text-xs text-gray-600 font-medium">{entry.category}</p>
                  </div>
                </div>

                {/* Right: GST details */}
                <div className="space-y-2.5">
                  <div className="bg-[#f8fbfd] border border-[#deeef7] rounded-xl p-3.5">
                    <p className="text-[10px] text-gray-400 font-semibold uppercase tracking-wide mb-2">GST Rate Details</p>
                    <div className="space-y-1.5">
                      ...
                      <div className="flex justify-between text-xs">
                        ...
                        <span className="font-bold text-gray-700">{entry.gstRate === 0 ? "Nil" : `${entry.gstRate / 2}%`}</span>
                      </div>
                      <div className="flex justify-between text-xs">
                      ...
                      <div className="flex justify-between text-xs">
                        ...
                        <span className="font-bold text-gray-700">{entry.gstRate === 0 ? "Nil" : `${entry.gstRate}%`}</span>
                      </div>
                      {entry.cess && (
                        <div className="flex justify-between text-xs border-t border-[#deeef7] pt-1.5 mt-1">
                          ...
                          <span className="font-bold text-orange-600">{entry.cess}</span>
                        </div>
                      )}
                    </div>
                  </div>
...
export default function HSNClient() {
  ...
  const hasActiveFilters =
    query !== "" || typeFilter !== "all" || rateFilter !== "all" || categoryFilter !== "all";
  ...
  const handleWhatsApp = () => {
    const msg = encodeURIComponent("Hello Taxvio! I need help finding the correct HSN/SAC code and GST rate for my goods/services. Can you assist?");
    ...
    <main className="min-h-screen bg-[#f8fbfd] font-sans text-gray-800">
        ...
        <div className="relative max-w-6xl mx-auto px-6 py-16 md:py-20">
          <motion.div initial="hidden" animate="visible" variants={stagger} className="text-center max-w-4xl mx-auto">
            ...
            <motion.span variants={fadeUp} className="inline-flex items-center gap-2 bg-white/10 border border-white/20 px-4 py-1.5 rounded-full text-xs font-semibold mb-5 backdrop-blur-sm uppercase tracking-wide">
              <Tag size={12} className="text-sky-300" />
              HSN Codes · SAC Codes · GST Rates · Free Tool
            </motion.span>
            ...
            <motion.p variants={fadeUp} className="mt-4 text-white/70 text-base md:text-lg leading-relaxed max-w-2xl mx-auto">
              Search and find HSN codes, SAC codes and applicable GST rates for any goods or service instantly. Updated for FY 2024-25.
            </motion.p>
            ...
            <motion.div variants={fadeUp} className="mt-8 max-w-2xl mx-auto">
              <div className="relative flex items-center bg-white rounded-2xl shadow-2xl shadow-black/20 overflow-hidden">
                <div className="pl-4 pr-2 py-4 shrink-0">
                  <Search size={20} className="text-[#00416a]" />
                </div>
                <input
                  ref={searchRef}
                  type="text"
                  ...
                  placeholder="Search by product, service, HSN/SAC code — e.g. laptop, medicine, 8471..."
                  className="flex-1 py-4 pr-4 text-gray-800 text-sm font-medium focus:outline-none placeholder:text-gray-400 bg-transparent"
                  autoComplete="off"
                />
                {query && (
                  <button onClick={() => handleSearch("")} className="pr-4 text-gray-400 hover:text-gray-600 transition">
                    <X size={16} />
                  </button>
                )}
                <button
                  onClick={() => {}}
                  className="mr-2 px-5 py-2.5 bg-[#00416a] text-white text-sm font-bold rounded-xl hover:bg-[#002b45] transition shrink-0"
                >
                  Search
                </button>
              </div>
              ...
              <motion.div variants={fadeUp} className="flex flex-wrap justify-center gap-2 mt-4">
                {["laptop", "medicine", "gold", "construction", "8517", "restaurant", "9971", "cotton"].map((s) => (
                  <button
                    key={s}
                    onClick={() => handleSearch(s)}
                    className="px-3 py-1.5 bg-white/10 border border-white/20 text-white/80 text-xs font-medium rounded-full hover:bg-white/20 hover:text-white transition"
                  >
                    {s}
                  </button>
                ))}
            ...
            <motion.div variants={fadeUp} className="mt-7 flex flex-wrap justify-center gap-3">
              {[
                ...
                { label: `${stats.goods} Goods (HSN)`, icon: Package },
                { label: `${stats.services} Services (SAC)`, icon: Layers },
                { label: "FY 2024-25 Updated", icon: BadgeCheck },
              ].map(({ label, icon: Icon }) => (
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6">
          <div className="grid lg:grid-cols-[1fr_340px] gap-6 items-start">
            ...
            <div>
              ...
              {/* ── Filters Bar ── */}
              <motion.div initial="hidden" animate="visible" variants={fadeUp} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 mb-5">
                <div className="flex flex-col gap-3">
                  {/* Top Row */}
                  <div className="flex items-center justify-between gap-2 flex-wrap">
                    <div className="flex items-center gap-2 flex-wrap">
                      {/* Type Toggle */}
                      <div className="flex items-center gap-1 p-1 bg-gray-100 rounded-xl text-xs">
                        {(["all", "goods", "services"] as const).map((t) => (
                          <button
                            key={t}
                            onClick={() => { setTypeFilter(t); setPage(1); }}
                            className={`px-3 py-1.5 rounded-lg font-semibold transition-all ${typeFilter === t ? "bg-[#00416a] text-white shadow-sm" : "text-gray-500 hover:text-gray-700"}`}
                          >
                            {t === "all" ? "All" : t === "goods" ? "📦 Goods" : "🛠️ Services"}
                          </button>
                        ))}
                      </div>
                      ...
                      <button
                        ...
                        className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold border transition-all ${showFilters ? "bg-[#00416a]/10 border-[#00416a]/30 text-[#00416a]" : "border-gray-200 text-gray-500 hover:border-gray-300"}`}
                      >
                        <Filter size={13} />
                        Filters
                        {(rateFilter !== "all" || categoryFilter !== "all") && (
                          <span className="w-4 h-4 bg-[#00416a] text-white rounded-full text-[9px] font-bold flex items-center justify-center">
                            {[rateFilter !== "all", categoryFilter !== "all"].filter(Boolean).length}
                          </span>
                        )}
                      </button>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-xs text-gray-500 font-medium">
                        <strong className="text-[#00416a]">{filtered.length}</strong> results
                      </span>
                      {hasActiveFilters && (
                        <button onClick={handleClearAll} className="flex items-center gap-1 text-xs text-red-500 hover:text-red-700 font-semibold transition">
                          <X size={12} /> Clear all
                        </button>
                      )}
                    </div>
                    ...
                    {showFilters && (
                      ...
                      >
                        <div className="grid sm:grid-cols-2 gap-3 pt-2 border-t border-gray-100">
                          {/* GST Rate */}
                          <div>
                            <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wide mb-2">Filter by GST Rate</p>
                            <div className="flex flex-wrap gap-1.5">
                              <button
                                onClick={() => { setRateFilter("all"); setPage(1); }}
                                className={`px-3 py-1.5 rounded-lg text-xs font-semibold border transition-all ${rateFilter === "all" ? "bg-[#00416a] text-white border-[#00416a]" : "border-gray-200 text-gray-600 hover:border-gray-300"}`}
                              >
                                All Rates
                              </button>
                              {GST_RATES.map((r) => (
                                <button
                                  key={r}
                                  onClick={() => { setRateFilter(r); setPage(1); }}
                                  className={`px-3 py-1.5 rounded-lg text-xs font-bold border transition-all ${rateFilter === r ? "bg-[#00416a] text-white border-[#00416a]" : `${RATE_COLORS[r]} hover:opacity-80`}`}
                                >
                                  {r === 0 ? "Exempt" : `${r}%`}
                                </button>
                              ))}
                          ...
                          {/* Category */}
                          <div>
                            <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wide mb-2">Filter by Category</p>
                            <select
              ...
              {paginated.length === 0 ? (
                ...
                >
                  ...
                  <p className="text-sm font-bold text-gray-600 mb-1">No codes found</p>
                  <p className="text-xs text-gray-400">
                    Try different keywords or{" "}
                    <button onClick={handleClearAll} className="text-[#00416a] font-semibold underline">clear all filters</button>
                  </p>
                </motion.div>
              ) : (
                <motion.div key={page} initial="hidden" animate="visible" variants={stagger} className="space-y-3">
                  {paginated.map((entry) => (
                    <motion.div key={entry.hsn} variants={fadeUp}>
                      <HSNCard
                        entry={entry}
                        isExpanded={expandedId === entry.hsn}
                        onToggle={() => setExpandedId(expandedId === entry.hsn ? null : entry.hsn)}
                      />
                    </motion.div>
                  ))}
                </motion.div>
              ...
              {totalPages > 1 && (
                <div className="flex items-center justify-between mt-6">
                  <button
                    ...
                    disabled={page === 1}
                    className="px-4 py-2 rounded-xl text-xs font-semibold border border-gray-200 text-gray-600 hover:border-[#00416a]/30 hover:text-[#00416a] disabled:opacity-40 disabled:cursor-not-allowed transition"
                  >
                    ← Previous
                  </button>
                    ...
                    {Array.from({ length: Math.min(totalPages, 7) }, (_, i) => {
                      ...
                      return (
                        <button
                          ...
                          className={`w-8 h-8 rounded-lg text-xs font-bold transition-all ${page === p ? "bg-[#00416a] text-white shadow-sm" : "text-gray-500 hover:bg-gray-100"}`}
                        >
                          {p}
                        </button>
                      );
            ...
            <div className="space-y-5 lg:sticky lg:top-24">
              ...
              <motion.div initial="hidden" animate="visible" variants={fadeUp} className="bg-[#00416a] rounded-3xl p-6 text-white relative overflow-hidden shadow-2xl shadow-[#00416a]/25">
                ...
                <div className="relative">
                  <p className="text-white/60 text-xs font-semibold uppercase tracking-wide mb-1">Database Overview</p>
                  <p className="text-2xl font-extrabold mb-4">{HSN_DATA.length}+ HSN / SAC Codes</p>

                  <div className="space-y-3">
                    {rateCounts.map(({ rate, count }) => (
                      <div key={rate}>
                        <div className="flex justify-between text-xs mb-1">
                          <span className="text-white/70">{rate === 0 ? "Exempt / NIL (0%)" : `${rate}% GST`}</span>
                        ...
                        <div className="h-1.5 bg-white/10 rounded-full overflow-hidden">
                          <div
                            ...
                            style={{ width: `${(count / HSN_DATA.length) * 100}%` }}
                          />
                        </div>
                      </div>
                    ))}
                  ...
                  <div className="mt-4 pt-4 border-t border-white/10 grid grid-cols-2 gap-3">
                    {[
                      ...
                      { label: "Services (SAC)", value: stats.services, color: "bg-teal-400" },
                      { label: "Exempt Items", value: stats.exempt, color: "bg-gray-400" },
                      { label: "Taxable Items", value: stats.taxable, color: "bg-orange-400" },
              ...
              {/* CTA Card */}
              <motion.div initial="hidden" animate="visible" variants={fadeUp} className="bg-white rounded-3xl border border-gray-100 shadow-sm p-5 space-y-3">
                <p className="text-sm font-bold text-[#00416a]">Need GST Filing Help?</p>
                <p className="text-xs text-gray-500 leading-relaxed">
                  Our CA team handles GST registration, GST return filing (GSTR-1, 3B, 9), HSN-wise reporting, e-invoicing setup, and GST audit — starting at ₹499/month.
                </p>
                <button
              ...
              <motion.div initial="hidden" animate="visible" variants={fadeUp} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                <p className="text-xs font-bold text-[#00416a] mb-3 flex items-center gap-2">
                  <BookOpen size={14} /> Quick GST Rate Reference
                </p>
                <div className="space-y-2">
                  {[
                    { rate: 0, example: "Fresh veg, healthcare, education", color: RATE_COLORS[0] },
                    { rate: 5, example: "Sugar, medicines, LPG, footwear ≤₹1K", color: RATE_COLORS[5] },
                    { rate: 12, example: "Computers, mobile phones, AC restaurants", color: RATE_COLORS[12] },
                    { rate: 18, example: "Electronics, telecom, IT services, banking", color: RATE_COLORS[18] },
                    { rate: 28, example: "Cars, AC, tobacco, aerated drinks, casinos", color: RATE_COLORS[28] },
                  ].map(({ rate, example, color }) => (
                    <button
                      key={rate}
                      onClick={() => { setRateFilter(rate); setPage(1); setExpandedId(null); }}
                      className="w-full flex items-center gap-2.5 p-2.5 rounded-xl border border-gray-100 hover:border-[#00416a]/20 hover:bg-[#f8fbfd] transition text-left"
                    >
                      <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold border shrink-0 ${color}`}>
                        {rate === 0 ? "NIL" : `${rate}%`}
                      </span>
                      <p className="text-[11px] text-gray-500 leading-tight">{example}</p>
                    </button>
                  ))}
                </div>
              </motion.div>
              ...
              {/* Tip */}
              <motion.div initial="hidden" animate="visible" variants={fadeUp} className="bg-amber-50 border border-amber-100 rounded-2xl p-4 flex items-start gap-3">
                <Info size={15} className="text-amber-600 shrink-0 mt-0.5" />
                <div>
                  <p className="text-xs font-bold text-amber-700">HSN Code Turnover Requirement</p>
                  <p className="text-[11px] mt-1 leading-relaxed text-amber-600">
                    <strong>≤ ₹1.5 Cr:</strong> HSN optional · <strong>₹1.5–5 Cr:</strong> 4-digit HSN · <strong>&gt; ₹5 Cr:</strong> 6-digit HSN mandatory on all invoices.
                  </p>
                </div>
              </motion.div>
            </div>
              ...
              <motion.div variants={fadeUp} className="grid md:grid-cols-2 gap-8 items-start">
                <div>
                  ...
                  <p className="text-gray-600 text-sm leading-relaxed mb-4">
                    ...
                    globally standardized 8-digit classification system developed by the
                    World Customs Organization (WCO), used in over 200 countries. In
                    India, HSN codes were adopted under GST to classify goods for
                  ...
                  <p className="text-gray-600 text-sm leading-relaxed">
                    For services, India uses <strong>SAC (Services Accounting Code)</strong>
                    — a similar classification system where all service codes begin with
                    '99' and are typically 6 digits long. SAC codes must appear on all
                    service invoices raised by GST-registered taxpayers.
                  </p>
                </div>
                <div className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
                  <div className="bg-[#00416a] px-5 py-3">
                    <p className="text-white font-bold text-sm">HSN Code Structure — Example: 8517 12 00</p>
                  </div>
                  <div className="divide-y divide-gray-100">
                    {[
                      ["Chapter (2-digit)", "85", "Electrical machinery & equipment"],
                      ["Heading (4-digit)", "8517", "Telephone sets, smartphones"],
                      ["Sub-heading (6-digit)", "8517 12", "Smartphones & mobile phones"],
                      ["Tariff Item (8-digit)", "8517 12 00", "Complete classification"],
                    ].map(([level, code, desc], i) => (
                      <div key={level} className={`grid grid-cols-3 px-4 py-3 text-xs ${i % 2 === 0 ? "bg-white" : "bg-gray-50/50"}`}>
                        <span className="font-semibold text-gray-600">{level}</span>
                        <span className="font-mono font-bold text-[#00416a] text-center">{code}</span>
                  ...
                  {[
                    {
                      ...
                      examples: ["Fresh vegetables & fruits", "Unbranded rice, wheat, pulses", "Fresh milk, eggs, honey", "Healthcare & hospital services", "Education up to degree level", "Sanitary napkins & diapers"],
                    ...
                    },
                    {
                      rate: "5% GST",
                      count: "60+ items",
                      examples: ["Sugar, edible oils, salt", "Branded rice & wheat flour", "Essential medicines", "Domestic LPG cylinders", "Economy class air tickets", "Footwear ≤ ₹1,000 MRP"],
                    ...
                    },
                    {
                      rate: "12% GST",
                      count: "55+ items",
                      examples: ["Computers & laptops", "Mobile phones", "Butter, cheese, ghee", "Frozen meats & fish", "Agricultural machinery", "Business class tickets"],
                    ...
                    },
                    {
                      rate: "18% GST",
                      count: "175+ items",
                      examples: ["Most electronics & appliances", "Telecom & IT services", "Financial & banking services", "Construction services", "AC restaurant bills", "Capital goods & machinery"],
                    ...
                    },
                    {
                      rate: "28% GST",
                      count: "35+ items",
                      examples: ["Luxury cars, SUVs, bikes", "Aerated drinks & tobacco", "Washing machines & ACs", "Dishwashers, large TVs", "Hotels > ₹7,500/night", "Online gaming & casinos"],
                    ...
                    },
                    {
                      rate: "3% GST",
                      count: "Special items",
                      examples: ["Gold, silver, platinum", "Coins of gold & silver", "Jewellery articles", "Goldsmiths' wares", "Precious metal articles", "Imitation jewellery"],
                      color: "bg-yellow-50 border-yellow-200",
                      badge: "bg-yellow-100 text-yellow-700",
                    },
                  ].map(({ rate, count, examples, color, badge }) => (
                    <div key={rate} className={`${color} border rounded-2xl p-4`}>
                      <div className="flex items-center justify-between mb-3">
                        <p className="text-sm font-extrabold text-[#00416a]">{rate}</p>
              ...
              <motion.div variants={fadeUp} className="grid md:grid-cols-2 gap-8">
                <div>
                  ...
                  <p className="text-gray-600 text-sm leading-relaxed mb-4">
                    ...
                    annual aggregate turnover. The requirement was further tightened for
                    e-invoicing and GSTR-1 HSN-wise summary (Table 12) reporting.
                    Non-compliance attracts a penalty of ₹50,000 (₹25,000 CGST +
                    ₹25,000 SGST) under Section 125 of CGST Act.
                  </p>
                  <div className="space-y-3">
                    {[
                      { step: "01", title: "Up to ₹1.5 Crore Turnover", desc: "Mentioning HSN code is optional. However, for B2B transactions and e-invoicing, it is recommended to use 4-digit HSN for clarity." },
                      { step: "02", title: "₹1.5 Crore to ₹5 Crore Turnover", desc: "4-digit HSN code mandatory on all B2B and B2C invoices. Must be reported in GSTR-1 Table 12 HSN summary on a quarterly/monthly basis." },
                      { step: "03", title: "Above ₹5 Crore Turnover", desc: "6-digit HSN code mandatory on all invoices. 8-digit codes required for export invoices and e-invoices under Rule 46 of CGST Rules." },
                      { step: "04", title: "E-Invoice Requirement", desc: "Businesses with turnover > ₹5 crore must generate e-invoices via IRP (Invoice Registration Portal). HSN codes are validated against CBIC master data during e-invoice generation." },
                    ].map(({ step, title, desc }) => (
                ...
                <div className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
                  <div className="bg-emerald-600 px-5 py-3">
                    <p className="text-white font-bold text-sm">GSTR-1 HSN Reporting — Table 12 Requirements</p>
                  </div>
                  <div className="divide-y divide-gray-100">
                    {[
                      ...
                      <div key={field} className={`grid grid-cols-2 px-4 py-2.5 text-xs ${i % 2 === 0 ? "bg-white" : "bg-gray-50/50"}`}>
                        <span className="font-semibold text-gray-700">{field}</span>
                        <span className="text-gray-500 text-right">{note}</span>
                      </div>
                    ))}
                  </div>
                  <div className="p-4 bg-blue-50">
                    <p className="text-xs text-blue-700 leading-relaxed">
                      <strong>Note:</strong> HSN-wise summary must be filed for each tax period in GSTR-1. Errors in HSN reporting attract audit scrutiny and ITC reversal notices.
                    </p>
                  </div>
                </div>
              </motion.div>
        ...
        <div className="max-w-5xl mx-auto px-6">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger}>
            <motion.div variants={fadeUp} className="text-center mb-10">
              ...
              <h2 className="text-3xl font-extrabold text-[#00416a] mt-4">Why Use Taxvio's HSN Code Finder?</h2>
            </motion.div>
            <motion.ul variants={stagger} className="grid md:grid-cols-2 gap-4">
              {[
                { icon: Search, text: "Instant full-text search across 500+ HSN and SAC codes — search by product name, code number, or category" },
                { icon: Filter, text: "Advanced filters — filter by goods/services, GST rate (0–28%), and category for pinpoint accuracy" },
                { icon: Tag, text: "Complete CGST/SGST/IGST breakdown for every code — know exact tax amounts for intra-state and inter-state supplies" },
                { icon: AlertCircle, text: "Important rate notes and exceptions clearly highlighted — know when special rates or exemptions apply" },
                { icon: Copy, text: "One-click copy HSN/SAC codes to clipboard — directly use in your invoices, returns, and software" },
                { icon: Shield, text: "Updated for FY 2024-25 with Budget 2024 changes — accurate GST rates including LTCG, compensation cess updates" },
                { icon: BadgeCheck, text: "CA-backed guidance — our GST experts verify classification disputes, wrong HSN usage, and GST notices" },
                { icon: FileText, text: "Full GST return filing support — GSTR-1, 3B, 9/9C, e-invoicing, HSN-wise reporting from ₹499/month" },
              ].map(({ icon: Icon, text }) => (
        ...
        <div className="max-w-3xl mx-auto px-6">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger}>
            <motion.div variants={fadeUp} className="text-center mb-10">
              ...
              <h2 className="text-3xl font-extrabold text-[#00416a] mt-4">HSN Code &amp; GST — Frequently Asked Questions</h2>
            </motion.div>
      ...
      <section className="py-14 bg-[#f8fbfd] border-t border-gray-100">
        <div className="max-w-6xl mx-auto px-6">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger}>
            <motion.p variants={fadeUp} className="text-xs font-bold uppercase tracking-widest text-[#00416a] mb-6">Related Tools &amp; Services</motion.p>
            <motion.div variants={stagger} className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {[
                { title: "GST Calculator", desc: "Calculate GST on any amount", href: "/tools/gst-calculator", icon: Calculator },
                { title: "EMI Calculator", desc: "Home, Car & Personal Loan EMI", href: "/tools/emi-calculator", icon: TrendingUp },
                { title: "GST Registration", desc: "GSTIN in 3–5 working days", href: "/serviceslist/gst/gst-registration", icon: Shield },
          ...
          <div className="relative bg-[#00416a] rounded-3xl overflow-hidden p-10 md:p-14 text-white text-center">
            ...
            <div className="relative">
              ...
              <p className="mt-4 text-white/70 max-w-xl mx-auto text-sm leading-relaxed">
                Taxvio's GST experts handle registration, return filing, HSN classification disputes, e-invoicing, e-way bills, annual returns (GSTR-9), and GST audit — complete GST compliance from ₹499/month.
              </p>
              <div className="mt-8 flex flex-col sm:flex-row items-center justify-center gap-3">
                <button onClick={handleWhatsApp} className="inline-flex items-center gap-2 bg-[#25d366] hover:bg-[#1ebe5d] text-white px-7 py-3.5 rounded-xl text-sm font-bold shadow-lg active:scale-[0.97] transition-all">
              ...
              <p className="mt-6 text-white/40 text-xs">
                GST Registration · GSTR-1/3B/9 · E-Invoice · HSN Classification · Audit Support · 100% Online
              </p>
            </div>
          </div>
        </div>
